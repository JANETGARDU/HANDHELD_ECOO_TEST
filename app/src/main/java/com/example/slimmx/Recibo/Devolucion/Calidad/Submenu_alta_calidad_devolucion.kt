package com.example.slimmx.Recibo.Devolucion.Calidad
...
import com.example.slimmx.Recibo.Devolucion.Calidad.Alta.Alta_Devolucion_Calidad
import com.example.slimmx.Recibo.Devolucion.Calidad.Verificacion.Calidad_devolucion_list
...
class Submenu_alta_calidad_devolucion : AppCompatActivity(), FragmentPageImpresionEtiquetas_Verificacion.OnBackgroundBlockerListener{
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_submenu_alta_calidad_devolucion)
        ...
        if (!mensaje.isNullOrEmpty()) {
            MensajesDialog.showMessage(this,"${mensaje}")
        }
        ...
        try {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                        ...
                        onSuccess = { lista ->
                            lifecycleScope.launch(Dispatchers.Main) {
                                if (lista.isNotEmpty()) {
                                    ...
                                    val intent =
                                        Intent(this@Submenu_alta_calidad_devolucion, Alta_Devolucion_Calidad::class.java)
                                    intent.putExtra("folios", opciones)
                                    startActivity(intent)
                                    LogsEntradaSalida.logsPorModulo( this@Submenu_alta_calidad_devolucion, lifecycleScope, "DEVOLUCION/CALIDAD/ALTA", "ENTRADA");
                                    finish()
                                } else {
                                    MensajesDialog.showMessage(this@Submenu_alta_calidad_devolucion, "No hay folios disponibles");
                                }
                            }
                        }, onError = {error->
                            lifecycleScope.launch(Dispatchers.Main) {
                                MensajesDialog.showMessage(this@Submenu_alta_calidad_devolucion, "${error}");
                            }
                    ...
                    lifecycleScope.launch(Dispatchers.Main) {
                        MensajesDialog.showMessage(this@Submenu_alta_calidad_devolucion, "Ocurrió un error: ${e.message}")
                    }
                }
            }
    ...
    private fun FoliosReciboCalidad() {
        try {
            ...
            lifecycleScope.launch(Dispatchers.IO) {
                try {
                    Pedir_datos_apis(
                        endpoint = "/devoluciones/calidad/recibo/folios",
                        params=emptyMap<String, String>(),
                        dataClass = ListReciboAbastecimiento::class,
                        listaKey = "result",
                        headers = headers,
                        onSuccess = { lista ->
                            lifecycleScope.launch(Dispatchers.Main) {
                                if (lista.isNotEmpty()) {
                                    val opciones = lista.map { it.FOLIO   }.toTypedArray()
                                    val intent =
                                        Intent(this@Submenu_alta_calidad_devolucion, Calidad_devolucion_list::class.java)
                                    intent.putExtra("folios", opciones)
                                    startActivity(intent)
                                    LogsEntradaSalida.logsPorModulo( this@Submenu_alta_calidad_devolucion, lifecycleScope, "DEVOLUCION/CALIDAD/RECIBO", "ENTRADA");
                                    finish()
                                } else {
                                    MensajesDialog.showMessage(this@Submenu_alta_calidad_devolucion, "No hay folios disponibles");
                                }
    ...
    override fun onBackPressed() {
        ...
        //LogsEntradaSalida.logsPorModulo( this, lifecycleScope, "DEVOLUCION/CALIDAD", "SALIDA");
        finish();
    }
